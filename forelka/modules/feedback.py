...
async def bot_feedback_handler(event):
...
async def feedback_exit_on_start(event) -> bool:
    """Called from the consolidated ``/start`` handler. Returns ``True`` if
    this handler has exited feedback mode for the user (so the caller should
    stop processing)."""
    kernel = event.client.kernel
    user_id = event.sender_id
...
def register(app, commands, module_name, kernel=None):
    ...
    if hasattr(kernel, 'inline_bot') and kernel.inline_bot and kernel.inline_bot.bot_client:
        ...
        print("[Feedback] Система обратной связи зарегистрирована.")
